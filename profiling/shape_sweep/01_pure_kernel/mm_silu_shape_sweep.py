#!/usr/bin/env python3
"""Pure-kernel shape sweep for MatMul and SiLU on RNGD.

Stage 1: measure isolated pure kernels while changing sequence length.

Usage:
  python mm_silu_shape_sweep.py
  python mm_silu_shape_sweep.py 128,256,512,1024,2048
"""
import csv
import os
...
OUT_CSV = os.path.join(HERE, "mm_silu_shape_sweep_results.csv")
...
class KernelBlock(torch.nn.Module):
...
def measure(kind, input_shape, weight_shape, dev):
    ...
    cm = CompileModule.from_exported(
        torch.export.export(KernelBlock(kind, weight), (x,))
    )
    cm.to(dev)
    xd = x.to(dev)
...
def main():
    ...
    rows = []
    for s in seq_lens:
        matmul = measure("matmul", (B, s, D), (D, INTER), dev)
        ...
        row = dict(
            ...
            isolated_sum_us=round(matmul["task_us"] + silu["task_us"], 2),
    ...
    fields = [
        "B", "S", "D", "INTER",
        "matmul_task_us", "silu_task_us", "isolated_sum_us",
        "matmul_dma_us", "silu_dma_us", "matmul_tu_us", "silu_tu_us",
    ]
